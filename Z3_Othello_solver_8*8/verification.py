...
class OthelloVerifier:
    ...
    def verify_termination(self):
        ...
        self.game.current_player = original_player  # Reset to original player
        
        # Game is over if the board is full or both players have no valid moves
        game_over = (total_disks == 64) or (not has_valid_moves and not opponent_has_moves)
        
        if game_over:
            self.verification_results["termination"] = {
                "status": "passed",
                "details": "Game has terminated. No more valid moves available for either player."
            }
        else:
            if not has_valid_moves:
                # Current player has no moves but opponent does - turn should be skipped
                self.verification_results["termination"] = {
                    "status": "pending",
                    "details": f"Current player has no valid moves. Turn should be skipped. Maximum {64 - total_disks} moves left."
                }
            else:
